def Sezar_(word:str, key:int, encrypt:bool = True):
    al = "2ezğwxtörvqb6ABCDEFGHIJKLMNOPQRSTUVWXYZdhlp1şf0ac3gəün9u7ym5osiıç8k4jŞÇƏİĞÖÜ"
    al = al + al.upper()

    if type(word) == str and type(key) == int and type(encrypt) == bool:
        if len(word) == 0: raise Exception("Söz minimumum 1 simvoldan ibarət olmalıdır!")

        if encrypt:
            new_word = "" 
            for letter in word:
                letter_index = al.find(letter) + key
                if letter_index > len(al) - 1:
                    letter_index = letter_index % len(al)
                new_word += al[letter_index]
            return new_word
        else: 
            new_word = ""
            for letter in word:
                letter_index = al.find(letter) - key 
                if letter_index < 0:
                    letter_index = letter_index + len(al)
                new_word += al[letter_index]
            return new_word
    else: raise Exception("Sezar funksiyası üçün göndərilmiş parametrlərin tipləri doğru deyil!")

def Sezar(word:str, key:int, encrypt:bool = True):
    try: return Sezar_(word=word, key=key, encrypt=encrypt)
    except Exception as msg: return msg 

class Person:
    caesar_key = 5
    user_count = 0
    person_list = list() 

    def __init__(self, name, surname):
        Person.user_count += 1
        self.id = Person.user_count
        self.name = name
        self.surname = surname 

        Person.person_list.append(self)

    def __str__(self): 
        return "{id},{name},{surname},{password}".format(
            id=self.id, 
            name=self.name, 
            surname=self.surname,
            password=self.__password)

    @property
    def get_password_encrypt(self): return self.__password

    @property
    def get_password(self): return Sezar(self.__password, Person.caesar_key, False)

    @get_password.setter
    def set_password(self, password):
        self.__password = Sezar(password, Person.caesar_key)
    

    @classmethod
    def To_CSV(klass):
        fayl = open("person.csv","w+", encoding="utf-8")
        fayl.write("ID,Name,Surname,Password\n") 
        for user in klass.person_list:  
            fayl.write(str(Person.__str__(user))+"\n")
        fayl.close()

class User(Person):
    def __init__(self, name, surname, username):
        super().__init__(name, surname)
        self.username = username
    
    def __str__(self):
        return "{id},{name},{surname},{username},{password}".format(
            id=self.id, 
            name=self.name, 
            surname=self.surname,
            username=self.username,
            password=self.get_password_encrypt)

    @classmethod
    def To_CSV(klass):
        fayl = open("users.csv","w+", encoding="utf-8")
        fayl.write("ID,Name,Surname,Username,Password\n") 
        for user in klass.person_list:  
            if isinstance(user, User):
                fayl.write(str(user)+"\n")
        fayl.close()


user_1 = User("Ali", "Hur", "alihur")
user_1.set_password = "1" 
user_1.__password = "2"
print(user_1) 


person_1 = Person("Alie", "Hure")
person_1.set_password = "1" 
print(person_1) 


Person.To_CSV()
User.To_CSV()